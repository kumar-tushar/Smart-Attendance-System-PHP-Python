...
height: 1.25rem;
  ...
  <body class="text-center">
	...
	<form class="form-signin" role="form"  name="add_new_subject_form" action="update-attendence.php"  method="POST">
	<img class="mb-4" src="https://getbootstrap.com/docs/4.0/assets/brand/bootstrap-solid.svg" alt="" width="72" height="72">
		...
		$menu_list_data = "SELECT * FROM `attendances` WHERE `rfid`=\"$rf_id\" AND `class`=\"$class\" AND `sub`=\"$sub\" ORDER BY `id` DESC";
		...
		while($row=mysqli_fetch_array($run))//while look to fetch the result and store in a array $row.
		{
			...
			$already_atendence = false;
			foreach($attendenceList as $atRollNo){
				if($atRollNo == $roll_no){
					$already_atendence = true;
					break;
				}
			}
			if($already_atendence == true){
				continue;
			}
			else{
				$found = true;
	...
	<a href="manage-attendence.php" class="btn btn-lg btn-primary btn-block">Home</a>
...
if(isset($_POST['submit_attendence']))
{
	...
	foreach($checklist as $checkd){
		array_push($attendenceList,$checkd);
	}
		
	$attendenceListJsonArry = json_encode($attendenceList);
	$querryattendence = str_replace('"','\"',$attendenceListJsonArry);// for inserting json replace " with \"
	...
	{
		$sql = "UPDATE `attendances` SET `attendence`=\"$querryattendence\" WHERE `date`=\"$date\" AND `class`=\"$class\" AND `sub`=\"$sub\" AND `rfid`=\"$rfid\"";
		$run = mysqli_query($dbcon,$sql);
		if(mysqli_affected_rows($dbcon) > 0){
			echo "<script>alert('Attedence update Sucessfully!')</script>";
			echo "<script>window.open('update-attendence.php','_self')</script>";
		}
		else{
			echo "<script>alert('Attedence update Fail!')</script>";
		}
	}
	else
	{
		
		echo "Attendance is not exist!";
